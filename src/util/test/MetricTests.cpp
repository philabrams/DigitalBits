...
using uniform_u64 = digitalbits::uniform_int_distribution<uint64_t>;
...
class SlidingWindowTester
{
  ...
  public:
    ...
    {
        mSlidingWindowSample.Seed(digitalbits::gRandomEngine());
    }
    ...
    addSamplesAtFrequency(std::chrono::milliseconds timeStep, Args... args)
    {
        ...
        {
            uint64_t sample =
                static_cast<uint64_t>(dist(digitalbits::gRandomEngine));
...
sampleFrom(Args... args)
{
    ...
    {
        sample.emplace_back(dist(digitalbits::gRandomEngine));
    }
...
testCKMSSample(int const count, Args... args)
{
    ...
    {
        auto x = static_cast<int64_t>(dist(digitalbits::gRandomEngine));
        values.push_back(x);
        hist.Update(x);
    }
