...
typedef std::minstd_rand digitalbits_default_random_engine;

extern digitalbits_default_random_engine gRandomEngine;

template <typename T>
T
rand_uniform(T lo, T hi)
{
    return digitalbits::uniform_int_distribution<T>(lo, hi)(gRandomEngine);
}
...
#ifdef BUILD_TESTS
// This function should be called any time you need to reset digitalbits-core's
