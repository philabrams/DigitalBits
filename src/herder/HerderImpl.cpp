...
void
HerderImpl::setTrackingSCPState(uint64_t index, DigitalBitsValue const& value,
...
void
HerderImpl::newSlotExternalized(bool synchronous, DigitalBitsValue const& value)
{
    ZoneScoped;
...
void
HerderImpl::processExternalized(uint64 slotIndex, DigitalBitsValue const& value)
{
    ZoneScoped;
...
void
HerderImpl::valueExternalized(uint64 slotIndex, DigitalBitsValue const& value,
                              bool isLatestSlot)
{
    ZoneScoped;
...
HerderImpl::checkCloseTime(SCPEnvelope const& envelope, bool enforceRecent)
{
    ...
    auto checkCTHelper = [&](std::vector<Value> const& values) {
        return std::any_of(values.begin(), values.end(), [&](Value const& e) {
            auto r = scpD.toDigitalBitsValue(e, sv);
                             ...
                             std::optional<SecretKey> skToSignValue)
{
    ...
    DigitalBitsValue sv =
        makeDigitalBitsValue(txSet->getContentsHash(), closeTime, upgrades, sk);
    ...
    getSCP().processSlotsAscendingFrom(ledgerSeq, [&](uint64 seq) {
        ...
        getSCP().processCurrentState(
            ...
            [&](SCPEnvelope const& e) {
                ...
                m.type(SCP_MESSAGE);
                m.envelope() = e;
                auto mPtr = std::make_shared<DigitalBitsMessage const>(m);
                              ...
                              bool checkTrackingSCP)
{
    ...
    // the exact closeTime contained in the DigitalBitsValue with the best
    ...
    DigitalBitsValue newProposedValue = makeDigitalBitsValue(
...
bool
HerderImpl::verifyDigitalBitsValueSignature(DigitalBitsValue const& sv)
{
    ZoneScoped;
    auto b = PubKeyUtils::verifySig(
...
DigitalBitsValue
HerderImpl::makeDigitalBitsValue(Hash const& txSetHash, uint64_t closeTime,
                             xdr::xvector<UpgradeType, 6> const& upgrades,
                             SecretKey const& s)
{
    ZoneScoped;
    DigitalBitsValue sv;
    sv.ext.v(DIGITALBITS_VALUE_SIGNED);
