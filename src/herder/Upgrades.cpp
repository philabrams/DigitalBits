...
template <class Archive>
void
load(Archive& ar, digitalbits::Upgrades::UpgradeParameters& o)
{
    time_t t;
    ar(make_nvp("time", t));
               ...
               AccountID const& accountID, Asset const& asset, int64_t delta)
{
    ...
    if (iter->second)
    {
        if (!digitalbits::addBalance(*iter->second, delta))
        {
            iter->second.reset();
        }
    }
                                        ...
                                        AbstractLedgerTxn& ltx)
{
    ...
    {
        auto trust = digitalbits::loadTrustLineWithoutRecord(ltx, accountID, asset);
    ...
    std::map<AccountID, std::vector<LedgerTxnEntry>> const& offersByAccount)
{
    ...
    for (auto const& accountOffers : offersByAccount)
    {
        ...
        auto accountEntry = digitalbits::loadAccount(ltx, accountID);
        if (!accountEntry)
        {
...
prepareLiabilities(AbstractLedgerTxn& ltx, LedgerTxnHeader const& header)
{
    ...
    for (auto& accountOffers : offersByAccount)
    {
        ...
        auto accountEntry = digitalbits::loadAccount(ltx, accountOffers.first);
        if (!accountEntry)
        {
            throw std::runtime_error("account does not exist");
        }
        ...
        for (auto const& assetLiabilities : liabilities)
        {
            ...
            {
                auto trustEntry =
                    digitalbits::loadTrustLine(ltx, accountOffers.first, asset);
                int64_t deltaSelling =
