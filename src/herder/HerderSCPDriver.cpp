...
void
HerderSCPDriver::restoreSCPState(uint64_t index, DigitalBitsValue const& value)
{
...
HerderSCPDriver::checkCloseTime(uint64_t slotIndex, uint64_t lastCloseTime,
                                DigitalBitsValue const& b) const
{
...
HerderSCPDriver::validateValueHelper(uint64_t slotIndex, DigitalBitsValue const& b,
                                     bool nomination) const
{
    uint64_t lastCloseTime;
    ZoneScoped;
    if (b.ext.v() == DIGITALBITS_VALUE_SIGNED)
    {
        ZoneNamedN(sigZone, "signature check", true);
        if (!mHerder.verifyDigitalBitsValueSignature(b))
        {
    ...
    bool const expectSignedValue =
        nomination || (compositeValueType() == DIGITALBITS_VALUE_SIGNED);
    if (!expectSignedValue && (b.ext.v() != DIGITALBITS_VALUE_BASIC))
    {
        CLOG_TRACE(Herder,
    ...
    if (expectSignedValue && (b.ext.v() != DIGITALBITS_VALUE_SIGNED))
    {
        CLOG_TRACE(Herder,
                   "HerderSCPDriver::validateValue i: {} invalid value type - "
                   "expected SIGNED",
...
HerderSCPDriver::getValueString(Value const& v) const
{
    ...
    try
    {
        ...
        return digitalbitsValueToString(mApp.getConfig(), b);
    }
    catch (...)
    {
        return "[:invalid:]";
                                   ...
                                   ValueWrapperPtrSet const& candidates)
{
    ...
    DigitalBitsValue comp(h, 0, emptyUpgradeSteps, DIGITALBITS_VALUE_BASIC);
    ...
    std::vector<DigitalBitsValue> candidateValues;
    ...
    for (auto const& c : candidates)
    {
        ...
        DigitalBitsValue& sv = candidateValues.back();
    ...
    if (!curProtocolPreservesTxSetCloseTimeAffinity())
    {
        ...
        comp.ext.v(DIGITALBITS_VALUE_BASIC);
    }
    comp.upgrades.clear();
    ...
    auto res = wrapDigitalBitsValue(comp);
    return res;
}

bool
HerderSCPDriver::toDigitalBitsValue(Value const& v, DigitalBitsValue& sv)
{
    try
    {
...
HerderSCPDriver::valueExternalized(uint64_t slotIndex, Value const& value)
{
    ...
    catch (...)
    {
        ...
        // therefore contain a valid DigitalBitsValue.
        CLOG_ERROR(Herder, "HerderSCPDriver::valueExternalized "
                           "Externalized DigitalBitsValue malformed");
...
void
HerderSCPDriver::nominate(uint64_t slotIndex, DigitalBitsValue const& value,
                          TxSetFramePtr proposedSet,
                          DigitalBitsValue const& previousValue)
{
    ZoneScoped;
    mCurrentValue = wrapDigitalBitsValue(value);
...
{
    return curProtocolPreservesTxSetCloseTimeAffinity() ? DIGITALBITS_VALUE_SIGNED
...
class SCPHerderValueWrapper : public ValueWrapper
{
  ...
  public:
    explicit SCPHerderValueWrapper(DigitalBitsValue const& sv, Value const& value,
                                   HerderImpl& herder)
...
{
    DigitalBitsValue sv;
    auto b = mHerder.getHerderSCPDriver().toDigitalBitsValue(val, sv);
    if (!b)
    {
        throw std::runtime_error(fmt::format(
...
ValueWrapperPtr
HerderSCPDriver::wrapDigitalBitsValue(DigitalBitsValue const& sv)
{
