...
class HerderSCPDriver : public SCPDriver
{
  public:
    ...
    void nominate(uint64_t slotIndex, DigitalBitsValue const& value,
                  TxSetFramePtr proposedSet, DigitalBitsValue const& previousValue);
    ...
    // converts a Value into a DigitalBitsValue
    // returns false on error
    bool toDigitalBitsValue(Value const& v, DigitalBitsValue& sv);
    ...
    // wraps a *valid* DigitalBitsValue (throws if it can't find txSet/qSet)
    ValueWrapperPtr wrapDigitalBitsValue(DigitalBitsValue const& sv);
