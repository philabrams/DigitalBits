...
// elsewhere in digitalbits-core so there's a little work up front converting
// representations.
//
...
class QuorumIntersectionCheckerImpl : public digitalbits::QuorumIntersectionChecker
{

    digitalbits::Config const& mCfg;
    ...
    // reporting.
    mutable std::pair<std::vector<digitalbits::NodeID>,
                      std::vector<digitalbits::NodeID>>
        mPotentialSplit;
    ...
    std::vector<digitalbits::NodeID> mBitNumPubKeys;
    std::unordered_map<digitalbits::NodeID, size_t> mPubKeyBitNums;
    QGraph mGraph;
    ...
    QBitSet convertSCPQuorumSet(digitalbits::SCPQuorumSet const& sqs);
    void buildGraph(digitalbits::QuorumTracker::QuorumMap const& qmap);
    void buildSCCs();
    ...
    mutable digitalbits::RandomEvictionCache<BitSet, bool, BitSet::HashFunction>
        mCachedQuorums;
  ...
  public:
    QuorumIntersectionCheckerImpl(digitalbits::QuorumTracker::QuorumMap const& qmap,
                                  digitalbits::Config const& cfg,
                                  std::atomic<bool>& interruptFlag,
                                  bool quiet = false);
    bool networkEnjoysQuorumIntersection() const override;

    std::pair<std::vector<digitalbits::NodeID>, std::vector<digitalbits::NodeID>>
