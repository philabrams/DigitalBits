...
class QuorumIntersectionChecker
{
  public:
    ...
    create(digitalbits::QuorumTracker::QuorumMap const& qmap,
           digitalbits::Config const& cfg, std::atomic<bool>& interruptFlag,
           bool quiet = false);

    static std::set<std::set<NodeID>>
    getIntersectionCriticalGroups(digitalbits::QuorumTracker::QuorumMap const& qmap,
