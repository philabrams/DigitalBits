...
bool force_sqlite = (std::getenv("DIGITALBITS_FORCE_SQLITE") != nullptr);
    ...
    if (!cfgs[instanceNumber])
    {
        if (gTestRoots.empty())
        {
            gTestRoots.emplace_back(
                fmt::format("digitalbits-core-test-{}", gBaseInstance));
        }
        ...
        thisConfig.FEE_POOL_PUBLIC_KEY = "GAEWWN53SHW7EICBTNKQH67ITUWQPSTBNTT44ZKHLQJFME6JETA664AC";
...
runTest(CommandLineArgs const& args)
{
    ...
    parser |= Catch::clara::Opt(gBaseInstance, "offset")["--base-instance"](
        ...
        "digitalbits-core can run tests concurrently");
    parser |=
    ...
    // not mixed with digitalbits-core logging.
    Logging::setFmt("<test>");
    Logging::setLogLevel(logLevel, nullptr);
    // use base instance for logging as we're guaranteed to not have conflicting
    // instances of digitalbits-core running at the same time with the same base
    // instance
    auto logFile = fmt::format("digitalbits{}.log", gBaseInstance);
    Logging::setLoggingToFile(logFile);
    Logging::setLogLevel(logLevel, nullptr);

    LOG_INFO(DEFAULT_LOG, "Testing digitalbits-core {}", DIGITALBITS_CORE_VERSION);
