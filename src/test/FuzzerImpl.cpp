...
struct xdr_fuzzer_compactor
{
    ...
    template <typename T>
    typename std::enable_if<std::is_same<digitalbits::AccountID, T>::value>::type
    operator()(T const& pk)
    {
        // convert public key 1 byte
        check(1);
        auto b = digitalbits::FuzzUtils::getShortKey(pk.ed25519());
        put_bytes(&b, 1);
    }
    template <typename T>
    typename std::enable_if<std::is_same<digitalbits::MuxedAccount, T>::value>::type
    operator()(T const& m)
    {
        // convert MuxedAccount -> 1 byte (same than an AccountID)
        auto const& ed25519 = (m.type() == digitalbits::KEY_TYPE_ED25519)
                                  ? m.ed25519()
                                  : m.med25519().ed25519;
        check(1);
...
struct xdr_fuzzer_unpacker
{
    ...
    operator()(T& pk)
    {
        ...
        digitalbits::FuzzUtils::setShortKey(pk, v);
    }
    template <typename T>
    typename std::enable_if<std::is_same<digitalbits::MuxedAccount, T>::value>::type
    operator()(T& m)
...
bool
isBadOverlayFuzzerInput(DigitalBitsMessage const& m)
{
...
OverlayFuzzer::genFuzz(std::string const& filename)
{
    ...
    out.exceptions(std::ios::failbit | std::ios::badbit);
    out.open(filename, std::ofstream::binary | std::ofstream::trunc);
    autocheck::generator<DigitalBitsMessage> gen;
    DigitalBitsMessage m(gen(FUZZER_INITIAL_CORPUS_MESSAGE_GEN_UPPERBOUND));
