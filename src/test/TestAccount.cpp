...
TestAccount::create(SecretKey const& secretKey, uint64_t initialBalance)
{
    ...
    {
        LedgerTxn ltx(mApp.getLedgerTxnRoot());
        auto entry = digitalbits::loadAccount(ltx, publicKey);
        if (entry)
        {
    ...
    {
        LedgerTxn ltx(mApp.getLedgerTxnRoot());
        auto destAfter = digitalbits::loadAccount(ltx, publicKey);
    ...
    {
        LedgerTxn ltx(mApp.getLedgerTxnRoot());
        REQUIRE(digitalbits::loadAccount(ltx, publicKey));
    }
...
TestAccount::merge(PublicKey const& into)
{
    ...
    LedgerTxn ltx(mApp.getLedgerTxnRoot());
    REQUIRE(digitalbits::loadAccount(ltx, into));
    REQUIRE(!digitalbits::loadAccount(ltx, getPublicKey()));
}
...
TestAccount::manageData(std::string const& name, DataValue* value)
{
    ...
    auto data = digitalbits::loadData(ls, getPublicKey(), name);
    if (value)
    {
        REQUIRE(data);
                                    ...
                                    xdr::xvector<Claimant, 10> const& claimants)
{
    ...
    LedgerTxn ltx(mApp.getLedgerTxnRoot());
    auto entry = digitalbits::loadClaimableBalance(ltx, returnedBalanceID);
    REQUIRE(entry);
...
TestAccount::pay(PublicKey const& destination, int64_t amount)
{
    ...
    {
        LedgerTxn ltx(mApp.getLedgerTxnRoot());
        auto toAccountEntry = digitalbits::loadAccount(ltx, destination);
        toAccount =
            toAccountEntry
    ...
    {
        LedgerTxn ltx(mApp.getLedgerTxnRoot());
        auto toAccountAfter = digitalbits::loadAccount(ltx, destination);
        // check that the target account didn't change
