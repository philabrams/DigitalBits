...
{
size_t
hash<digitalbits::MergeKey>::operator()(digitalbits::MergeKey const& key) const noexcept
{
    std::ostringstream oss;
    oss << key.mKeepDeadEntries << ','
        << digitalbits::binToHex(key.mInputCurrBucket) << ','
        << digitalbits::binToHex(key.mInputSnapBucket);
    for (auto const& e : key.mInputShadowBuckets)
    {
        oss << digitalbits::binToHex(e) << ',';
    }
