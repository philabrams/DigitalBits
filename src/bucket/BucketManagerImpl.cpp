    ...
    catch (std::exception const& e)
    {
        throw std::runtime_error(fmt::format(
            FMT_STRING("{}. This can be caused by access rights issues or "
                       "another digitalbits-core process already running"),
            e.what()));
    }
...
const std::string BucketManagerImpl::kLockFilename = "digitalbits-core.lock";
...
BucketManagerImpl::deleteTmpDirAndUnlockBucketDir()
{
    ...
    // Then delete the lockfile $BUCKET_DIR_PATH/digitalbits-core.lock
    if (mLockedBucketDir)
    {
        std::string d = mApp.getConfig().BUCKET_DIR_PATH;
        std::string lock = d + "/" + kLockFilename;
