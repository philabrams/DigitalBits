...
getSecureCreds(std::string const& prompt)
{
    ...
    dwErr = CredUIPromptForCredentials(
        ...
        TEXT("DigitalBits"),                   // Target for credentials
                                           //   (usually a server)
        NULL,                              // Reserved
        0,                                 // Reason
        pszName,                           // User name
...
signtxn(std::string const& filename, std::string netId, bool base64)
{
    ...
    {
        if (netId.empty())
            netId = getenv("DIGITALBITS_NETWORK_ID");
        if (netId.empty())
            throw std::runtime_error("missing --netid argument or "
                                     "DIGITALBITS_NETWORK_ID environment variable");
        ...
        readFile(filename, base64, [&](xdr::opaque_vec<> d) {
            ...
            case ENVELOPE_TYPE_TX_V0:
                ...
                // TransactionV0 and Transaction always have the same signatures
                // so there is no reason to check versions here, just always
