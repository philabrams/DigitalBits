...
CommandLine::ConfigOption::getConfig(bool logToFile) const
{
    Config config;
    auto configFile =
        mConfigFile.empty() ? std::string{"digitalbits-core.cfg"} : mConfigFile;
...
runForceSCP(CommandLineArgs const& args)
{
    ...
    auto resetOption = clara::Opt{reset}["--reset"](
        "reset force SCP flag, so next time digitalbits-core "
        ...
        [&] {
            ...
            try
            {
                ...
                if (cfg.MANUAL_CLOSE)
                {
                    ...
                    {
                        LOG_ERROR(DEFAULT_LOG, "Starting digitalbits-core in "
...
{
    std::cout << DIGITALBITS_CORE_VERSION << std::endl;
    return 0;
}
...
handleCommandLine(int argc, char* const* argv)
{
    auto commandLine = CommandLine{
        {{"catchup",
         ...
         {"http-command", "send a command to local digitalbits-core",
          runHttpCommand},
         ...
         {"run", "run digitalbits-core node", run},
