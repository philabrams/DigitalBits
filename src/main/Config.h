...
class Config : public std::enable_shared_from_this<Config>
{
  ...
  public:
    ...
    // captive core. This is useful for testing how digitalbits-core buffers ledgers
    // during captive core fast restart.
    std::chrono::seconds ARTIFICIALLY_DELAY_BUCKET_APPLICATION_FOR_TESTING;

    // A config parameter that forces digitalbits-core to sleep every time a task is
    ...
    uint32_t TESTING_UPGRADE_DESIRED_FEE; // in nibbs
    uint32_t TESTING_UPGRADE_RESERVE;     // in nibbs
    ...
    std::string FEE_PASSPHRASE; // identifier for the fees pool account, deprecated
    std::string FEE_POOL_PUBLIC_KEY; // identifier for the fees pool account
    ...
    digitalbits::SCPQuorumSet QUORUM_SET;
