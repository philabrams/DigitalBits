...
runWithConfig(Config cfg, optional<CatchupConfiguration> cc)
{
    ...
    if (cfg.MANUAL_CLOSE)
    {
        if (!cfg.NODE_IS_VALIDATOR)
        {
            LOG_ERROR(DEFAULT_LOG,
                      "Starting digitalbits-core in MANUAL_CLOSE mode requires "
    ...
    LOG_INFO(DEFAULT_LOG, "Starting digitalbits-core {}", DIGITALBITS_CORE_VERSION);
        ...
        Json::Value& catchupInfo, std::shared_ptr<HistoryArchive> archive)
{
    ...
    catch (std::invalid_argument const&)
    {
        ...
        LOG_INFO(
            DEFAULT_LOG,
            "* If you really want to catchup to {} run digitalbits-core new-db",
            cc.toLedger());
        LOG_INFO(DEFAULT_LOG, "*");
        return 2;
    }
