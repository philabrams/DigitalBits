...
TEST_CASE("liabilities", "[ledger][liabilities]")
{
    ...
    SECTION("add account selling liabilities")
    {
        auto addSellingLiabilitiesHelper = [&](uint32_t initNumSubEntries,
            ...
            bool res =
                digitalbits::addSellingLiabilities(header, acc, deltaLiabilities);
        ...
        auto addSellingLiabilitiesUninitialized =
            [&](uint32_t initNumSubEntries, int64_t initBalance,
                ...
                auto header = ltx.loadHeader();
                auto acc = ltx.create(le);
                bool res = digitalbits::addSellingLiabilities(header, acc,
                                                          deltaLiabilities);
                REQUIRE(acc.current().data.account().balance == initBalance);
    ...
    SECTION("add account buying liabilities")
    {
        auto addBuyingLiabilitiesHelper = [&](uint32_t initNumSubEntries,
            ...
            auto acc = ltx.create(le);
            bool res =
                digitalbits::addBuyingLiabilities(header, acc, deltaLiabilities);
            REQUIRE(acc.current().data.account().balance == initBalance);
            REQUIRE(getSellingLiabilities(header, acc) ==
                    initSellingLiabilities);
            if (res)
    ...
    SECTION("add trustline selling liabilities")
    {
        auto addSellingLiabilities = [&](int64_t initLimit, int64_t initBalance,
            ...
            bool res =
                digitalbits::addSellingLiabilities(header, trust, deltaLiabilities);
        ...
        auto addSellingLiabilitiesUninitialized =
            [&](int64_t initLimit, int64_t initBalance,
                ...
                auto header = ltx.loadHeader();
                auto trust = ltx.create(le);
                bool res = digitalbits::addSellingLiabilities(header, trust,
                                                          deltaLiabilities);
    ...
    SECTION("add trustline buying liabilities")
    {
        auto addBuyingLiabilities = [&](int64_t initLimit, int64_t initBalance,
            ...
            auto trust = ltx.create(le);
            bool res =
                digitalbits::addBuyingLiabilities(header, trust, deltaLiabilities);
            REQUIRE(trust.current().data.trustLine().balance == initBalance);
            REQUIRE(getSellingLiabilities(header, trust) ==
                    initSellingLiabilities);
            if (res)
...
TEST_CASE("balance with liabilities", "[ledger][liabilities]")
{
    ...
    SECTION("account add balance")
    {
        auto addBalanceHelper =
            [&](uint32_t initNumSubEntries, uint32_t initNumSponsoring,
                ...
                auto header = ltx.loadHeader();
                auto acc = ltx.create(le);
                bool res = digitalbits::addBalance(header, acc, deltaBalance);
                REQUIRE(getSellingLiabilities(header, acc) ==
    ...
    SECTION("trustline add balance")
    {
        auto addBalance = [&](int64_t initLimit, int64_t initBalance,
            ...
            auto header = ltx.loadHeader();
            auto trust = ltx.create(le);
            bool res = digitalbits::addBalance(header, trust, deltaBalance);
            REQUIRE(getSellingLiabilities(header, trust) ==
                    initLiabilities.selling);
            REQUIRE(getBuyingLiabilities(header, trust) ==
                    initLiabilities.buying);
...
TEST_CASE("available balance and limit", "[ledger][liabilities]")
{
    ...
    SECTION("account available balance")
    {
        auto checkAvailableBalanceHelper = [&](uint32_t initNumSubEntries,
            ...
            REQUIRE(!digitalbits::addBalance(header, acc, -availableBalance - 1));
            REQUIRE(digitalbits::addBalance(header, acc, -availableBalance));
        };
    ...
    SECTION("account available limit")
    {
        auto checkAvailableLimitHelper = [&](uint32_t initNumSubEntries,
            ...
            {
                REQUIRE(!digitalbits::addBalance(header, acc, availableLimit + 1));
            }
            REQUIRE(digitalbits::addBalance(header, acc, availableLimit));
        };
    ...
    SECTION("trustline available balance")
    {
        auto checkAvailableBalance = [&](int64_t initLimit, int64_t initBalance,
            ...
            REQUIRE(!digitalbits::addBalance(header, trust, -availableBalance - 1));
            REQUIRE(digitalbits::addBalance(header, trust, -availableBalance));
        };
    ...
    SECTION("trustline available limit")
    {
        auto checkAvailableLimit = [&](int64_t initLimit, int64_t initBalance,
            ...
            {
                REQUIRE(
                    !digitalbits::addBalance(header, trust, availableLimit + 1));
            }
            REQUIRE(digitalbits::addBalance(header, trust, availableLimit));
        };
