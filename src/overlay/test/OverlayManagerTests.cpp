...
class OverlayManagerTests
{
  ...
  protected:
    ...
    testBroadcast()
    {
        ...
        DigitalBitsMessage AtoB = a.tx({payment(b, 10)})->toDigitalBitsMessage();
        auto i = 0;
        for (auto p : pm.mOutboundPeers.mAuthenticated)
        {
            if (i++ == 2)
        ...
        crank(10);
        REQUIRE(sentCounts(pm) == expected);
        DigitalBitsMessage CtoD = c.tx({payment(d, 10)})->toDigitalBitsMessage();
        ...
        DigitalBitsMessage AtoC = a.tx({payment(c, 10)})->toDigitalBitsMessage();
