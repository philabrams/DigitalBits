                      ...
                      public NonMovableOrCopyable
{
  public:
    ...
    void relayOrProcessResponse(DigitalBitsMessage const& msg, Peer::pointer peer);
    void relayOrProcessRequest(DigitalBitsMessage const& msg, Peer::pointer peer);
    void clearOldLedgers(uint32_t lastClosedledgerSeq);
    Json::Value const& getJsonResults();

    static std::string getMsgSummary(DigitalBitsMessage const& msg);
    DigitalBitsMessage makeSurveyRequest(NodeID const& nodeToSurvey) const;
  ...
  private:
    ...
    void broadcast(DigitalBitsMessage const& msg) const;
