...
void
Peer::beginMesssageProcessing(DigitalBitsMessage const& msg)
{
...
Peer::sendHello()
{
    ...
    auto msgPtr = std::make_shared<DigitalBitsMessage const>(msg);
    sendMessage(msgPtr);
}
...
Peer::sendGetTxSet(uint256 const& setID)
{
    ...
    auto msgPtr = std::make_shared<DigitalBitsMessage const>(newMsg);
    sendMessage(msgPtr);
}
...
{
    ZoneScoped;
    DigitalBitsMessage m;
    m.type(ERROR_MSG);
    m.error().code = error;
    m.error().msg = message;
    auto msgPtr = std::make_shared<DigitalBitsMessage const>(m);
    sendMessage(msgPtr);
}
...
std::string
Peer::msgSummary(DigitalBitsMessage const& msg)
{
    switch (msg.type())
    {
    case ERROR_MSG:
...
void
Peer::sendMessage(std::shared_ptr<DigitalBitsMessage const> msg, bool log)
{
    ZoneScoped;
...
void
Peer::sendAuthenticatedMessage(DigitalBitsMessage const& msg)
{
...
void
Peer::recvMessage(DigitalBitsMessage const& digitalbitsMsg)
{
    ZoneScoped;
    if (shouldAbort())
    {
    ...
    case SEND_MORE:
    {
        ...
        {
            if (digitalbitsMsg.sendMoreMessage().numMessages == 0)
            {
        ...
        if (digitalbitsMsg.sendMoreMessage().numMessages >
        ...
        if (mOutboundCapacity == 0 &&
            digitalbitsMsg.sendMoreMessage().numMessages != 0)
        {
            CLOG_DEBUG(Overlay, "Got outbound capacity for peer {}",
                       mApp.getConfig().toShortString(getPeerID()));
        }
        mOutboundCapacity += digitalbitsMsg.sendMoreMessage().numMessages;
        break;
    }
    ...
    if (!mApp.getLedgerManager().isSynced() && digitalbitsMsg.type() == TRANSACTION)
    {
    ...
    auto msgTracker = std::make_shared<MsgCapacityTracker>(weak, digitalbitsMsg);
...
void
Peer::recvSendMore(DigitalBitsMessage const& msg)
{
    ZoneScoped;
...
void
Peer::addMsgAndMaybeTrimQueue(std::shared_ptr<DigitalBitsMessage const> msg)
{
    ZoneScoped;
...
void
Peer::endMessageProcessing(DigitalBitsMessage const& msg)
{
    if (shouldAbort() ||
...
void
Peer::recvRawMessage(DigitalBitsMessage const& digitalbitsMsg)
{
    ZoneScoped;
    ...
    if (!isAuthenticated() && (digitalbitsMsg.type() != HELLO) &&
        (digitalbitsMsg.type() != AUTH) && (digitalbitsMsg.type() != ERROR_MSG))
    {
    ...
    releaseAssert(isAuthenticated() || digitalbitsMsg.type() == HELLO ||
                  digitalbitsMsg.type() == AUTH || digitalbitsMsg.type() == ERROR_MSG);
    mApp.getOverlayManager().recordMessageMetric(digitalbitsMsg,
                                                 shared_from_this());
    ...
    case HELLO:
    {
        ...
        this->recvHello(digitalbitsMsg.hello());
    }
    break;
    ...
    case SURVEY_REQUEST:
    {
        ...
        recvSurveyRequestMessage(digitalbitsMsg);
    }
    break;
    ...
    case SURVEY_RESPONSE:
    {
        ...
        recvSurveyResponseMessage(digitalbitsMsg);
    }
    break;
    ...
    case TRANSACTION:
    {
        ...
        recvTransaction(digitalbitsMsg);
    }
    break;
    ...
    case GET_SCP_QUORUMSET:
    {
        ...
        recvGetSCPQuorumSet(digitalbitsMsg);
    }
    break;
    ...
    case SCP_QUORUMSET:
    {
        ...
        recvSCPQuorumSet(digitalbitsMsg);
    }
    break;
    ...
    case GET_SCP_STATE:
    {
        ...
        recvGetSCPState(digitalbitsMsg);
    }
    break;
    case SEND_MORE:
    {
        auto t = getOverlayMetrics().mRecvSendMoreTimer.TimeScope();
        recvSendMore(digitalbitsMsg);
    }
    break;
    }
}
...
void
Peer::recvDontHave(DigitalBitsMessage const& msg)
{
    ZoneScoped;
...
void
Peer::recvGetTxSet(DigitalBitsMessage const& msg)
{
    ZoneScoped;
    auto self = shared_from_this();
    if (auto txSet = mApp.getHerder().getTxSet(msg.txSetHash()))
    {
        ...
        auto newMsgPtr = std::make_shared<DigitalBitsMessage const>(newMsg);
...
void
Peer::recvTxSet(DigitalBitsMessage const& msg)
{
    ZoneScoped;
...
void
Peer::recvTransaction(DigitalBitsMessage const& msg)
{
    ZoneScoped;
...
void
Peer::recvGetSCPQuorumSet(DigitalBitsMessage const& msg)
{
    ZoneScoped;
...
void
Peer::recvSCPQuorumSet(DigitalBitsMessage const& msg)
{
    ZoneScoped;
...
void
Peer::recvSCPMessage(DigitalBitsMessage const& msg)
{
    ZoneScoped;
...
void
Peer::recvGetSCPState(DigitalBitsMessage const& msg)
{
    ZoneScoped;
...
void
Peer::recvError(DigitalBitsMessage const& msg)
{
    ZoneScoped;
...
void
Peer::recvAuth(DigitalBitsMessage const& msg)
{
    ZoneScoped;
    if (mState != GOT_HELLO)
    {
...
void
Peer::recvGetPeers(DigitalBitsMessage const& msg)
{
    ZoneScoped;
    sendPeers();
}
...
void
Peer::recvPeers(DigitalBitsMessage const& msg)
{
    ZoneScoped;
...
void
Peer::recvSurveyRequestMessage(DigitalBitsMessage const& msg)
{
    ZoneScoped;
...
void
Peer::recvSurveyResponseMessage(DigitalBitsMessage const& msg)
{
    ZoneScoped;
