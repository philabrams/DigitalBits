...
void
Peer::sendMessage(DigitalBitsMessage const& msg, bool log)
{
    ZoneScoped;
...
Peer::recvMessage(DigitalBitsMessage const& digitalbitsMsg)
{
    ...
    mApp.postOnMainThread(
        [
            weak, sm = DigitalBitsMessage(digitalbitsMsg), mtype = digitalbitsMsg.type(),
...
Peer::recvRawMessage(DigitalBitsMessage const& digitalbitsMsg)
{
    ...
    assert(isAuthenticated() || digitalbitsMsg.type() == HELLO ||
           digitalbitsMsg.type() == AUTH || digitalbitsMsg.type() == ERROR_MSG);
