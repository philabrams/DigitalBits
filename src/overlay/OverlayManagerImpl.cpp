...
bool
OverlayManagerImpl::recvFloodedMsgID(DigitalBitsMessage const& msg,
...
void
OverlayManagerImpl::recordMessageMetric(DigitalBitsMessage const& digitalbitsMsg,
                                        Peer::pointer peer)
{
    ZoneScoped;
    ...
    else if (digitalbitsMsg.type() != TX_SET && digitalbitsMsg.type() != SCP_QUORUMSET)
    {
        return;
    }
    ...
    size_t size = xdr::xdr_argpack_size(digitalbitsMsg);
    auto hash = shortHash::xdrComputeHash(digitalbitsMsg);
...
void
OverlayManagerImpl::updateFloodRecord(DigitalBitsMessage const& oldMsg,
