              ...
              LedgerTxnEntry& acc, int64_t delta)
{
    ...
    {
        if (!digitalbits::addBalance(header, acc, delta))
        {
            throw std::runtime_error("insufficient balance");
        }
    }
...
LiquidityPoolDepositOpFrame::doApply(AbstractLedgerTxn& ltx)
{
    ...
    if (!digitalbits::addBalance(cp().reserveA, amountA, INT64_MAX))
    {
        throw std::runtime_error("insufficient liquidity pool limit");
    }
    updateBalance(header, tlB, source, -amountB);
    if (!digitalbits::addBalance(cp().reserveB, amountB, INT64_MAX))
    {
        throw std::runtime_error("insufficient liquidity pool limit");
    }

    if (!digitalbits::addBalance(header, tlPool, amountPoolShares))
    {
        throw std::runtime_error("insufficient pool share limit");
    }
    if (!digitalbits::addBalance(cp().totalPoolShares, amountPoolShares, INT64_MAX))
    {
        throw std::runtime_error("insufficient liquidity pool limit");
    }
