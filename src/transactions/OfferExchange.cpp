           ...
           std::vector<ClaimAtom>& offerTrail)
{
    ...
    {
        auto accountB = digitalbits::loadAccountWithoutRecord(ltx, accountBID);
        if (!accountB)
        {
    ...
    { // entire offer is taken
        auto accountB = digitalbits::loadAccount(ltx, accountBID);
    ...
    if (numSheepSend != 0)
    {
        ...
        if (sheep.type() == ASSET_TYPE_NATIVE)
        {
            auto accountB = digitalbits::loadAccount(ltxInner, accountBID);
        ...
        {
            auto sheepLineAccountB =
                digitalbits::loadTrustLine(ltxInner, accountBID, sheep);
    ...
    if (numWheatReceived != 0)
    {
        ...
        {
            auto wheatLineAccountB =
                digitalbits::loadTrustLine(ltxInner, accountBID, wheat);
              ...
              RoundingType round, std::vector<ClaimAtom>& offerTrail)
{
    ...
    if (!digitalbits::loadAccountWithoutRecord(ltx, accountBID))
    {
        throw std::runtime_error(
            "invalid database state: offer must have matching account");
    }
    ...
    {
        accountB = digitalbits::loadAccount(ltx, accountBID);
    }
