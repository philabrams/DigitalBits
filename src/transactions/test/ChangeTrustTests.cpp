...
TEST_CASE("change trust", "[tx][changetrust]")
{
        ...
        SECTION("edit existing")
        {
            ...
            for_all_versions(*app, [&] {
                                  ...
                                  ex_CHANGE_TRUST_NO_ISSUER);
                {
                    ...
                    REQUIRE(!digitalbits::loadAccount(ltx, gateway));
                }
            });
        }
    }
    SECTION("trusting self")
    {
        auto validateTrustLineIsConst = [&]() {
            LedgerTxn ltx(app->getLedgerTxnRoot());
            auto trustLine =
                digitalbits::loadTrustLine(ltx, gateway.getPublicKey(), idr);
            REQUIRE(trustLine);
        ...
        auto loadAccount = [&](PublicKey const& k) {
            LedgerTxn ltx(app->getLedgerTxnRoot());
            auto le = digitalbits::loadAccount(ltx, k).current();
          ...
          "[tx][changetrust][liquiditypool]")
{
    ...
    SECTION("pool trustline")
    {
        auto getNumSubEntries = [&](AccountID const& accountID) {
            LedgerTxn ltx(app->getLedgerTxnRoot());
            auto acc = digitalbits::loadAccount(ltx, accountID);
