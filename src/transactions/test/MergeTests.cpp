...
TEST_CASE("merge", "[tx][merge]")
{
    ...
    SECTION("success")
    {
        SECTION("success - basic")
        {
            for_all_versions(*app, [&] {
...

                {
                    ...
                    REQUIRE(!digitalbits::loadAccount(ltx, a1.getPublicKey()));
                }
            });
        }
        SECTION("success, invalidates dependent tx")
        {
            for_all_versions(*app, [&] {
                ...
                auto a1Balance = a1.getBalance();
                auto b1Balance = b1.getBalance();
                // can this be a problem or something else???
                // Change 3 to 4
    ...
    SECTION("account has only base reserve + one nibb")
    {
        auto mergeFrom = root.create(
    ...
    SECTION("account has only base reserve + one operation fee - one nibb")
    {
        auto mergeFrom = root.create(
            "merge-from",
    ...
    SECTION("account has only base reserve + two operation fees - one nibb")
    {
        auto mergeFrom = root.create(
            "merge-from",
    ...
    SECTION("destination with native buying liabilities")
    {
        ...
        for_versions_from(10, *app, [&] {
...

            {
                ...
                auto account = digitalbits::loadAccount(ltx, acc1.getPublicKey());
