...
TEST_CASE("pathpayment strict send", "[tx][pathpayment]")
{
    ...
    SECTION("not supported before version 12")
    {
        for_versions_to(11, *app, [&] {
            REQUIRE_THROWS_AS(
                root.pathPaymentStrictSend(root, xdb, 1, xdb, 1, {}),
                ex_opNOT_SUPPORTED);
        });
    }
        ...
        for_versions_from(12, *app, [&] {
            ...
            for (int64_t amount : std::vector<int64_t>({0, -1}))
            {
                REQUIRE_THROWS_AS(source.pathPaymentStrictSend(
                                      destination, xdb, amount, xdb, 1, {}),
                                  ex_PATH_PAYMENT_STRICT_SEND_MALFORMED);
                // clang-format off
                market.requireBalances(
                    {{source, {{xdb, minBalance1 - (++i) * txfee}}},
                     {destination, {{xdb, minBalance}}}});
                // clang-format on
            }
        });
    }
    ...
    SECTION("currency invalid")
    {
        ...
        for_versions_from(12, *app, [&] {
            ...
            // clang-format off
            market.requireBalances(
                {{source, {{xdb, minBalance1 - txfee}}},
                 {destination, {{xdb, minBalance}}}});
            // clang-format on

            REQUIRE_THROWS_AS(source.pathPaymentStrictSend(destination, xdb, 10,
                                                           makeInvalidAsset(),
                                                           10, {}),
                              ex_PATH_PAYMENT_STRICT_SEND_MALFORMED);
            // clang-format off
            market.requireBalances(
                {{source, {{xdb, minBalance1 - 2 * txfee}}},
                 {destination, {{xdb, minBalance}}}});
            // clang-format on

            REQUIRE_THROWS_AS(
                source.pathPaymentStrictSend(destination, xdb, 10, xdb, 10,
                                             {makeInvalidAsset()}),
                ex_PATH_PAYMENT_STRICT_SEND_MALFORMED);
            // clang-format off
            market.requireBalances(
                {{source, {{xdb, minBalance1 - 3 * txfee}}},
                 {destination, {{xdb, minBalance}}}});
            // clang-format on
        });
    }
    ...
    SECTION("send amount too big")
    {
        ...
        for_versions_from(12, *app, [&] {
            REQUIRE_THROWS_AS(a.pathPaymentStrictSend(
                                  dest, xdb, minBalance1 + 1, xdb, 20, {}),
                              ex_PATH_PAYMENT_STRICT_SEND_UNDERFUNDED);
            // clang-format off
            market.requireBalances(
                {{a, {{xdb, minBalance1 - 2 * txfee}, {idr, 10}}},
                 {dest, {{xdb, minBalance1 - txfee}, {idr, 0}}}});
            // clang-format on
            REQUIRE_THROWS_AS(
            ...
            // clang-format off
            market.requireBalances(
                {{a, {{xdb, minBalance1 - 3 * txfee}, {idr, 10}}},
                 {dest, {{xdb, minBalance1 - txfee}, {idr, 0}}}});
            // clang-format on
        });
    }
    ...
    SECTION("source does not have trustline")
    {
        ...
        for_versions_from(12, *app, [&] {
            ...
            // clang-format off
            market.requireBalances(
                {{noSourceTrust, {{xdb, minBalance - txfee}, {idr, 0}}},
                 {destination, {{xdb, minBalance1 - txfee}, {idr, 0}}}});
            // clang-format on
            REQUIRE_THROWS_AS(noSourceTrust.pathPaymentStrictSend(
            ...
            // clang-format off
            market.requireBalances(
                {{noSourceTrust, {{xdb, minBalance - 2 * txfee}, {idr, 0}}},
                 {destination, {{xdb, minBalance1 - txfee}, {idr, 0}}}});
            // clang-format on
        });
    }
    ...
    SECTION("source is not authorized")
    {
        ...
        for_versions_from(12, *app, [&] {
            ...
            // clang-format off
            market.requireBalances(
                {{noAuthorizedSourceTrust, {{xdb, minBalance1 - 2 * txfee}, {idr, 10}}},
                 {destination, {{xdb, minBalance1 - txfee}, {idr, 0}}}});
            // clang-format on
            REQUIRE_THROWS_AS(noAuthorizedSourceTrust.pathPaymentStrictSend(
            ...
            // clang-format off
            market.requireBalances(
                {{noAuthorizedSourceTrust, {{xdb, minBalance1 - 3 * txfee}, {idr, 10}}},
                 {destination, {{xdb, minBalance1 - txfee}, {idr, 0}}}});
            // clang-format on
        });
    }
    ...
    SECTION("destination does not exist")
    {
        ...
        for_versions_from(12, *app, [&] {
            ...
            // clang-format off
            market.requireBalances(
                {{source, {{xdb, minBalance1 - 2 * txfee}, {idr, 10}}}});
            // clang-format on
        });
    }
    ...
    SECTION("destination is issuer and does not exist for simple paths")
    {
        ...
        for_versions_from(12, *app, [&] {
            ...
            // clang-format off
            market.requireBalances(
                {{source, {{xdb, minBalance1 - 2 * txfee}, {idr, 0}}}});
            // clang-format on
        });
    }
    ...
    SECTION("destination does not have trustline")
    {
        ...
        for_versions_from(12, *app, [&] {
            ...
            // clang-format off
            market.requireBalances(
                {{source, {{xdb, minBalance1 - txfee}, {idr, 10}}},
                 {noDestinationTrust, {{xdb, minBalance}, {idr, 0}}}});
            // clang-format on
        ...
        for_versions_from(12, *app, [&] {
            ...
            market.requireBalances(
                {{source, {{xdb, minBalance1 - txfee}, {idr, 10}, {usd, 0}}},
                 {noAuthorizedDestinationTrust, {{xdb, minBalance1 - txfee}, {idr, 0}}}});
            // clang-format on
            REQUIRE_THROWS_AS(
    ...
    SECTION("destination line full")
    {
        ...
        for_versions_from(12, *app, [&] {
            ...
            // clang-format off
            market.requireBalances(
                {{source, {{xdb, minBalance1 - txfee}, {idr, 11}, {usd, 0}}},
            ...
            // clang-format off
            market.requireBalances(
                {{source, {{xdb, minBalance1 - 2 * txfee}, {idr, 11}, {usd, 0}}},
                 {destination, {{xdb, minBalance1 - txfee}, {idr, 10}, {usd, 0}}}});
            // clang-format on
        });
    }
    ...
    SECTION("under destination minimum XDB")
    {
        auto market = TestMarket{*app};
        ...
        for_versions_from(12, *app, [&] {
            REQUIRE_THROWS_AS(
                source.pathPaymentStrictSend(destination, xdb, 10, xdb, 11, {}),
    ...
    SECTION("to self XDB")
    {
        ...
        for_versions_from(12, *app, [&] {
            account.pathPaymentStrictSend(account, xdb, 20, xdb, 20, {});
    ...
    SECTION("takes best offers, multiple offers per exchange")
    {
        ...
        for_versions_from(12, *app, [&] {
            ...
            // clang-format off
            market.requireBalances(
                {{source, {{xdb, minBalance4 - 2 * txfee}, {cur1, 3}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12, {{xdb, minBalance5 - 5 * txfee}, {cur1, 55}, {cur2, 2}, {cur3, 0}, {cur4, 0}}},
                 {mm23, {{xdb, minBalance5 - 5 * txfee}, {cur1, 0}, {cur2, 33}, {cur3, 1}, {cur4, 0}}},
                 {mm34, {{xdb, minBalance5 - 5 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 22}, {cur4, 1}}},
        ...
        for_versions_from(12, *app, [&] {
            ...
            market.requireBalances(
                {{source, {{xdb, minBalance4 - 2 * txfee}, {cur1, 3}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12, {{xdb, minBalance5 - 5 * txfee}, {cur1, 55}, {cur2, 2}, {cur3, 0}, {cur4, 0}}},
                 {mm23, {{xdb, minBalance5 - 5 * txfee}, {cur1, 0}, {cur2, 33}, {cur3, 1}, {cur4, 0}}},
                 {mm34, {{xdb, minBalance5 - 4 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 21}, {cur4, 1}}},
                 {mm34b, {{xdb, minBalance5 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 1}, {cur4, 1}}},
                 {destination, {{xdb, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 15}}}});
            // clang-format on
        });
    }
    ...
    SECTION("with rounding errors")
    {
        ...
        for_versions_from(12, *app, [&] {
            auto sellerOfferRemaining =
                OfferState{cny, xdb, price, 145000000 - 20039999};
            auto actual = std::vector<ClaimOfferAtom>{};
            market.requireChanges(
                {{sellerOffer.key, sellerOfferRemaining}}, [&] {
                    actual =
                        source
                            .pathPaymentStrictSend(destination, xdb, 1382068965,
            ...
            market.requireBalances(
                {{source, {{xdb, 1989999000 - 100 - 1382068965}}},
