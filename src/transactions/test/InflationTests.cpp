                   ...
                   std::function<int(int)> getVote)
{
    ...
    for (int i = 0; i < nbAccounts; i++)
    {
        ...
        if (bal >= 0)
        {
            ...
            auto account = digitalbits::loadAccount(ltx, to.getPublicKey());
                  ...
                  std::function<int(int)> getVote, Application& app)
{
    ...
    for (auto w : winners)
    {
        ...
        if (protocolVersionStartsFrom(ledgerVersion, ProtocolVersion::V_10))
        {
            ...
            auto winner =
                digitalbits::loadAccount(ltx, getTestAccount(w).getPublicKey());
            toDoleToThis =
            ...
            std::function<int(int)> getVote, size_t expectedWinnerCount)
{
    ...
    for (int i = 0; i < nbAccounts; i++)
    {
        ...
        else
        {
            LedgerTxn ltx(app.getLedgerTxnRoot());
            auto account =
                digitalbits::loadAccount(ltx, getTestAccount(i).getPublicKey());
    ...
    for (int i = 0; i < nbAccounts; i++)
    {
        ...
        else
        {
            {
                LedgerTxn ltx(app.getLedgerTxnRoot());
                auto account = digitalbits::loadAccount(ltx, k.getPublicKey());
                auto const& ae = account.current().data.account();
