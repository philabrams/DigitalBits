                ...
                SECTION("single signature")
                {
                    ...
                    SECTION("accountMerge signing account")
                    {
                        ...
                        for_versions_from(3, *app, [&] {
                            ...
                            {
                                LedgerTxn ltx(app->getLedgerTxnRoot());
                                REQUIRE(!digitalbits::loadAccount(ltx, b1));
                            }
                        });
                    }
                    ...
                    SECTION("failing transaction")
                    {
                        ...
                        for_versions(3, 9, *app, [&] {
                            setup();
                            applyCheck(tx, *app);
                            REQUIRE(tx->getResultCode() == digitalbits::txFAILED);
                            REQUIRE(PaymentOpFrame::getInnerCode(getFirstResult(
                                        *tx)) == digitalbits::PAYMENT_MALFORMED);
                            REQUIRE(getAccountSigners(a1, *app).size() == 1);
                        });
                        for_versions_from(10, *app, [&] {
                            setup();
                            ...
                            alternative.name)
                    {
                        for_versions_from(14, *app, [&] {
                            ...
                            if (alternative.autoRemove)
                            {
                                ...
                                auto ltxe = digitalbits::loadAccount(ltx, a1);
                                auto const& a1Entry =
        ...
        SECTION("reduce auth, do something")
        {
            ...
            SECTION("multiple tx")
            {
                for_versions_from(10, *app, [&] {
                    ...
                    auto r =
                        closeLedgerOn(*app, 3, 1, 2, 2016, {tx1, tx2}, true);
