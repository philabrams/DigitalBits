...
TEST_CASE("payment", "[tx][payment]")
{
    ...
    SECTION("send XDB to an existing account")
    {
        for_all_versions(*app, [&] {
            ...
            {
                LedgerTxn ltx(app->getLedgerTxnRoot());
                auto rootAccount2 = digitalbits::loadAccount(ltx, root);
                auto rootAcc2 = rootAccount2.current().data.account();
                auto a1Account2 = digitalbits::loadAccount(ltx, a1);
    ...
    SECTION("send XDB to a new account (no destination)")
    {
        for_all_versions(*app, [&] {
            REQUIRE_THROWS_AS(
    ...
    SECTION("rescue account (was below reserve)")
    {
        for_all_versions(*app, [&] {
            auto getMinBalance = [&] {
                LedgerTxn ltx(app->getLedgerTxnRoot());
                return digitalbits::getMinBalance(ltx.loadHeader().current(), 0, 0,
                                              0);
            };
    ...
    SECTION("create, path payment, merge, create")
    {
        ...
        for_all_versions(*app, [&] {
            auto tx = sourceAccount.tx({
                ...
                createDestination.op(pathPayment(payDestination, xdb, payAmount,
                                                 xdb, payAmount, {})),
            ...
            for (auto const& data : withoutTrustLine)
            {
                SECTION(data.name)
                {
                    SECTION("without trustline")
                    {
                        ...
                        REQUIRE(!digitalbits::loadTrustLine(ltx, sendToSelf,
                                                        data.asset));
                    }
                }
            }
            ...
            for (auto const& data : withTrustLine)
            {
                SECTION(data.name)
                {
                    SECTION("with trustline and 0 balance")
                    {
                        ...
                        auto trust =
                            digitalbits::loadTrustLine(ltx, sendToSelf, data.asset);
                        REQUIRE(trust);
    ...
    SECTION("liabilities")
    {
        SECTION("cannot pay balance below selling liabilities")
        {
            ...
            auto setup = [&]() {
                ...
                auto offer = market.requireChangesWithOffer({}, [&] {
                    return market.addOffer(a1, {idr, xdb, Price{1, 1}, 50});
                });
            };
                ...
                " limit")
        {
            ...
            auto setup = [&]() {
                TestMarket market(*app);
                auto offer = market.requireChangesWithOffer({}, [&] {
                    return market.addOffer(a1, {xdb, idr, Price{1, 1}, 50});
                });
            };
            for_versions_to(9, *app, [&] {
                setup();
    ...
    SECTION("dest amount too big for native asset")
    {
        for_versions_to(10, *app, [&] {
            REQUIRE_THROWS_AS(
                root.pay(a1, xdb, std::numeric_limits<int64_t>::max()),
                ex_txINTERNAL_ERROR);
        });
