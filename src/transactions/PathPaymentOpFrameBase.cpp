...
PathPaymentOpFrameBase::checkIssuer(AbstractLedgerTxn& ltx, Asset const& asset)
{
    if (asset.type() != ASSET_TYPE_NATIVE)
    {
        ...
        if (protocolVersionIsBefore(ledgerVersion, ProtocolVersion::V_13) &&
            !digitalbits::loadAccountWithoutRecord(ltx, getIssuer(asset)))
        {
                                            ...
                                            bool doesSourceAccountExist)
{
    ...
    if (asset.type() == ASSET_TYPE_NATIVE)
    {
        ...
        if (protocolVersionStartsFrom(header.current().ledgerVersion,
                                      ProtocolVersion::V_8))
        {
            sourceAccount = digitalbits::loadAccount(ltx, getSourceID());
            if (!sourceAccount)
            {
                setResultMalformed();
                return false;
                                          ...
                                          bool bypassIssuerCheck)
{
    ...
    if (asset.type() == ASSET_TYPE_NATIVE)
    {
        auto destination = digitalbits::loadAccount(ltx, destID);
    ...
    else
    {
        ...
        auto destLine = digitalbits::loadTrustLine(ltx, destID, asset);
        if (!destLine)
        {
