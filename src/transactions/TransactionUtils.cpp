                            ...
                            LedgerTxnEntry const& offerEntry, bool isAcquire)
{
    ...
    auto loadAccountAndValidate = [&ltx, &sellerID]() {
        auto account = digitalbits::loadAccount(ltx, sellerID);
        if (!account)
        {
    ...
    auto loadTrustAndValidate = [&ltx, &sellerID](Asset const& asset) {
        auto trust = digitalbits::loadTrustLine(ltx, sellerID, asset);
        if (!trust)
        {
...
addBalance(LedgerTxnHeader const& header, LedgerTxnEntry& entry, int64_t delta)
{
    if (entry.current().data.type() == ACCOUNT)
    {
        ...
        if (!digitalbits::addBalance(newBalance, delta))
        {
            return false;
        }
        if (protocolVersionStartsFrom(header.current().ledgerVersion,
                     ...
                     int64_t delta)
{
    ...
    if (entry.current().data.type() == ACCOUNT)
    {
        ...
        bool res = digitalbits::addBalance(buyingLiab, delta, maxLiabilities);
        if (res)
        {
                      ...
                      int64_t delta)
{
    ...
    if (entry.current().data.type() == ACCOUNT)
    {
        ...
        bool res = digitalbits::addBalance(sellingLiab, delta, maxLiabilities);
        if (res)
        {
...
struct MuxChecker
{
    ...
    void
    operator()(digitalbits::MuxedAccount const& t)
    {
