...
ManageOfferOpFrameBase::checkOfferValid(AbstractLedgerTxn& ltxOuter)
{
    ...
    if (mSheep.type() != ASSET_TYPE_NATIVE)
    {
        ...
        {
            auto issuer = digitalbits::loadAccount(ltx, getIssuer(mSheep));
            if (!issuer)
            {
    ...
    if (mWheat.type() != ASSET_TYPE_NATIVE)
    {
        ...
        if (protocolVersionIsBefore(ledgerVersion, ProtocolVersion::V_13))
        {
            auto issuer = digitalbits::loadAccount(ltx, getIssuer(mWheat));
            if (!issuer)
            {
...
ManageOfferOpFrameBase::doApply(AbstractLedgerTxn& ltxOuter)
{
    ...
    { // modifying an old offer
        auto sellSheepOffer = digitalbits::loadOffer(ltx, getSourceID(), mOfferID);
        if (!sellSheepOffer)
        {
            setResultNotFound();
            return false;
    ...
    if (!isDeleteOffer())
    {
                                      ...
                                      ProtocolVersion::V_10))
        {
            ...
            {
                auto sourceAccount =
                    digitalbits::loadAccountWithoutRecord(ltx, getSourceID());
