...
You can check if the build that you're using has them if `digitalbits-core help` returns help for the `test` subcommand:
```
...
or using the `http-command` sub-command `digitalbits-core http-command peers`
...
In order to mitigate this risk, digitalbits-core can emit a "reference file" full of trusted hashes for
...
`METADATA_OUTPUT_STREAM` allows a digitalbits-core instance to stream meta data instead
...
Last history checkpoint {
    "version": 1,
    "server": "digitalbits-core 12.3.0 (5a8f8b13af5f78704bf0e7b5de4314c16a3639a5)",
    "currentLedger": 89663,
    "currentBuckets": [
        {
...
```
digitalbits-core print-xdr --base64 tx-result.txt
TransactionResult = {
  feeCharged = 100,
  result = {
    code = txSUCCESS,
...
You can generate a new DigitalBits public/private key with the `gen-seed` command.

```
digitalbits-core.exe gen-seed
...
```
digitalbits-core convert-id GDJLE5FAS4RVCWXAXUU226TGKXCAX7WFRNVCP75BNPWOUW7QSLKZZTUH
PublicKey:
