...
This document describes changes to the DigitalBits protocol as well as other changes made to address security issues.
...
* Changes to SCP.
* Changes to the DigitalBits Protocol (anything that modifies how the distributed ledger functions, including historical data).

## Security issues

* DDoS.
* Crashes (that could lead to remote code execution).
* Other attacks that can be exploited (inside or outside of the DigitalBits protocol).

## Goals for this document

* have a summary view of changes that affect the code base (digitalbits-core has to be able to replay all ledgers generated since genesis on the DigitalBits public network).
