...
digitalbits-core is at the bottom of the DigitalBits stack, many other services can be
 ...
 Example of such services include Frontier, a service to assist with
 submitting transactions to the network as well as exposing a data model to
 query ledger history.

 In the rest of this document we'll use Frontier to better illustrate data flows.
...
* core's SQL database (latest only, updated every ledger)
  * in the case of captive-core (enabled via the `--in-memory` command line option) the ledger is maintained within the digitalbits-core process and ledger-state need to be tracked as it changes via "meta" updates.
...
In captive-core mode, this entire per-ledger transition dataset is emitted by digitalbits-core over a (optionally named) pipe, as an xdr encoded `LedgerCloseMeta` object.
...
With such view, systems like Frontier can:
...
When starting fresh, before being able to use ledger changes, the processor (Frontier) needs to initialize its initial state.
...
* developers of Frontier have to
  ...
  *  be extremely careful when performing requests as to avoid
     *  inconsistent queries
        *  digitalbits-core may change ledger between or while Frontier is making queries
...
* operationally, this creates coupling between digitalbits-core and Frontier:
  * a traffic spike in Frontier may cause enough stress on digitalbits-core that it loses sync
  * it's hard to "scale out" Frontier
