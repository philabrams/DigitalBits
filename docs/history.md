...
This database is consulted and updated "live" in an ACID fashion as digitalbits-core applies
...
In total, each checkpoint number `0xwwxxyyzz` consists of the following files:
  ...
  - One ledger-headers file, named by ledger number as `ledger/ww/xx/yy/ledger-wwxxyyzz.xdr.gz`. The
    ...
    [`LedgerHeaderHistoryEntry`](/src/xdr/DigitalBits-ledger.x), one per ledger in the checkpoint (so
  ...
  - One transactions file, named by ledger number as
    ...
    structures of the type [`TransactionHistoryEntry`](/src/xdr/DigitalBits-ledger.x), with zero-or-more
  ...
  - One results file, named by ledger number as `results/ww/xx/yy/results-wwxxyyzz.xdr.gz`. The file
    ...
    [`TransactionHistoryResultEntry`](/src/xdr/DigitalBits-ledger.x), with zero-or-more structures per
    ...
    to the ledger without actually running the `digitalbits-core` transaction-apply logic.
  ...
  - (Optionally) one SCP file, named by ledger number as `scp/ww/xx/yy/scp-wwxxyyzz.xdr.gz`. The file
    contains a sequence of XDR structures of the type
    [`SCPHistoryEntry`](/src/xdr/DigitalBits-ledger.x), with zero-or-more structures per ledger. The
